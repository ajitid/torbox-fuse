...
# KDE Wayland workaround for Plex HTPC:
# Force Plex HTPC's Qt launcher to use the X11/xcb backend, which runs through XWayland.
# This can avoid Plex HTPC's blurry/pixelated native Wayland rendering on KDE mixed-DPI/fractional-scale setups.

DESKTOP_FILE="/usr/share/applications/tv.plex.PlexHTPC.desktop"

usage() {
  cat <<EOF
Usage:
  $0            Patch Plex HTPC desktop launcher to force QT_QPA_PLATFORM=xcb
...
if [[ "$REVERT" == 1 ]]; then
  if [[ -f "${DESKTOP_FILE}.bak" ]]; then
    ...
    echo 'OK: reverted Plex HTPC KDE launcher patch. Restart Plex HTPC fully.'
  else
...
if grep -q '^Exec=env QT_QPA_PLATFORM=xcb Plex' "$DESKTOP_FILE"; then
  echo 'OK: Plex HTPC desktop launcher now forces QT_QPA_PLATFORM=xcb.'
  echo 'Restart Plex HTPC fully for changes to take effect.'
else
